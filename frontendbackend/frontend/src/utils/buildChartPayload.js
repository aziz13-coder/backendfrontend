...
export function buildChartPayload(chart, includeVerdict = true) {
  ...
  const payload = {
    ...
    // Export the engine's structured reasoning used for the verdict
    reasoning: engineReasoning,
    // Include evaluation diagnostics when present
    ...(chart.scoring_trace ? { scoring_trace: chart.scoring_trace } : {}),
