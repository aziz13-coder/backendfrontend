...
const EnhancedChartWheel = ({ chart, darkMode }) => {
  ...
  if (cusps.length === 12) {
    for (let i = 0; i < 12; i++) {
      // Rotate so Aries is at the top (−90°)
      const cur = normalizeAngle(cusps[i] - 90);
      const nxt = normalizeAngle(cusps[(i + 1) % 12] - 90);

      // Smallest positive delta from cur → nxt
      const delta = normalizeAngle(nxt - cur);
      const end = cur + delta; // end angle relative to cur
      ...
      const mid = cur + delta / 2;

      houseSectors.push({
        houseNumber: i + 1,
        startAngle: cur,
        endAngle: end,
        pathData: `M 192 192 L ${startX} ${startY} A 140 140 0 ${largeArc} 1 ${endX} ${endY} Z`,
        midAngle: mid,
