...
class TraditionalHoraryQuestionAnalyzer:
    ...
    def analyze_question(self, question: str) -> Dict[str, Any]:
        ...
        question_intent = self._determine_question_intent(question)
    ...
    def _determine_question_type(self, question: str) -> tuple[Category, List[str]]:
        ...
        # PRIORITY 0: Property investment questions - check BEFORE generic financial transactions
        property_investment_patterns = [
            (r'\b(invest|investment)\s+in\s+(property|real\s*estate|house|building|land)\b', "property investment"),
            (r'\b(buy|purchase|buying)\s+(property|real\s*estate|house|building|land)\b', "property purchase"),
            (r'\bshould\s+i\s+(invest|buy).*\b(property|real\s*estate|house|building|land)\b', "property decision"),
            (r'\b(property|real\s*estate|house|building|land).*\b(invest|investment|buy|purchase)\b', "property transaction")
        ]
        
        import re
        for pattern, description in property_investment_patterns:
        ...
        has_possession = any(word in question.lower() for word in possession_indicators)
        has_pet = any(word in question.lower() for word in pet_keywords)
        
        if has_possession and has_pet:
            matched_pets = [word for word in pet_keywords if word in question.lower()]
    ...
    def _determine_question_intent(self, question: str) -> str:
        """Determine if question is about SAFETY/condition vs REUNION/recovery"""
        safety_keywords = ["safe", "alive", "harmed", "okay", "condition", "well", "healthy", "hurt", "injured", "sick", "dead", "died"]
        reunion_keywords = ["find", "recover", "return", "contact", "get back", "come back", "reunite", "locate", "where"]
        
        question_lower = question.lower()
        
        has_safety = any(word in question_lower for word in safety_keywords)
        has_reunion = any(word in question_lower for word in reunion_keywords)
        
        if has_safety and not has_reunion:
            return "SAFETY"
        elif has_reunion and not has_safety:
            return "REUNION"
        elif has_safety and has_reunion:
            # Both present - prioritize based on question structure
            # "Is my missing cat safe?" = SAFETY (focus on condition)
            # "Will I find my missing cat?" = REUNION (focus on recovery)
            if any(q_word in question_lower for q_word in ["is", "are", "will they be", "is he", "is she"]):
                return "SAFETY"
            else:
                return "REUNION"
        else:
            # Neither explicit - infer from context
            if any(word in question_lower for word in ["missing", "lost"]):
                return "SAFETY"  # Default missing items to safety questions
            return "REUNION"
